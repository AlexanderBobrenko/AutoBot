...
2. В Railway добавьте переменные окружения:
   ...
   - `USD_RUB_RATE`
   - `EUR_RUB_RATE`
   - `GOOGLE_CREDENTIALS_JSON` (если планируете использовать `/connect_google`)
...
Команда `/connect_google` использует файл `credentials.json` в корне проекта рядом с `bot.py`.

На Railway удобнее хранить JSON сервисного аккаунта в переменной окружения `GOOGLE_CREDENTIALS_JSON`: бот сам создаст `credentials.json` при первом вызове `/connect_google`.

Если вы не задали `GOOGLE_CREDENTIALS_JSON` и не положили `credentials.json` в репозиторий/папку на хостинге, используйте `/connect_google` не сможете.
...
- `/connect_google` — синхронизация с Google Sheets (`credentials.json` или `GOOGLE_CREDENTIALS_JSON`)
